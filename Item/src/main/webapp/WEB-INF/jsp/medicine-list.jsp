...
<nav class="breadcrumb"> <a href="/index.jsp">首页</a>  <span class="c-gray en">&gt;</span> 药品管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<form id="sform" name="sform" action="/medicine/search">
<div class="pd-20">
  <input type="hidden" id="pageNum" name="pageNum" value="${pageInfo.pageNum}">
  <div class="text-c">
    <input type="text" class="input-text" style="width:550px" placeholder="输入药品名称关键字" id="name" name="name" value="${name}"><button type="submit" class="btn btn-success" id="" name=""> 搜药品</button>

  </div>
  <div class="cl pd-5 bg-1 bk-gray mt-20">
   <%-- <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="icon-trash"></i> 批量删除</a>
   --%> <a href="/medicine-add.jsp" onclick="user_add('550','','添加药品','/medicine-add.jsp')" class="btn btn-primary radius"><i class="icon-plus"></i> 添加药品</a></span>
  ...
  <table class="table table-border table-bordered table-hover table-bg table-sort">
    ...
    <c:forEach var="medicines" items="${pageInfo.list}">
      <tr class="text-c">
        <%--<td><input type="checkbox" value="1" name=""></td>--%>
        <td>${medicines.MId}</td>

        <td><u style="cursor:pointer" class="text-primary" onclick="user_show('10001','360','','张三','user-show.html')">${medicines.MName}</u></td>
        <td>${medicines.MType}</td>
        <td>${medicines.MDescript}</td>
        <td><fmt:formatDate value="${medicines.productionDate}" pattern="yyyy-MM-dd"  /></td>
        <td>${medicines.expiryDate}</td>
        <td>${medicines.norms}</td>
        <td>${medicines.MImg}</td>
        <td>${medicines.MQuantity}</td>
        <td>${medicines.bid}</td>
        ...
        <td><a style="text-decoration: none" class="ml-5" onClick=""
               href="update?id=${medicines.MId}" title="编辑">编辑
                ...
                href="delete?id=${medicines.MId}" title="删除">删除 </a></td>
      </tr>
    </c:forEach>
    </tbody>
...
<DIV class=pager>
  <UL>
    <LI class=current><A href="javascript:void(0)" onclick="dosubmit(pageNum=${pageInfo.firstPage})">首页</A></LI>
