...
<nav class="breadcrumb"> <a href="/index.jsp">首页</a>   <span class="c-gray en">&gt;</span> 订单管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<form id="sform" name="sform" action="/order/search">
<div class="pd-20">
  <input type="hidden" id="pageNum" name="pageNum" value="${pageInfo.pageNum}">
  <div class="text-c">
    <input type="text" class="input-text" style="width:550px" placeholder="输入用户名关键字" id="name" name="name" value="${name}"><button type="submit" class="btn btn-success" id="" name=""> 搜订单</button>
    ...
    <c:forEach var="order" items="${pageInfo.list}">
      <tr class="text-c">
       <%-- <td><input type="checkbox" value="1" name=""></td>--%>
        ...
        <td><u style="cursor:pointer" class="text-primary" onclick="user_show('10001','360','','张三','user-show.html')">${order.userName}</u></td>
        ...
        <td><%--<a style="text-decoration: none" class="ml-5" onClick=""
               href="update?id=${medicines.MId}" title="编辑">编辑
                ...
                href="delete?id=${order.orderId}" title="删除">删除 </a></td>
      </tr>
    </c:forEach>
    </tbody>
